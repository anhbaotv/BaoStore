...
const UploadAppModal: React.FC<UploadAppModalProps> = ({ isOpen, onClose, onUpload }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!appName || !icon || !description || !installUrl) {
      setError('Vui lòng điền đầy đủ thông tin.');
      return;
    }
    onUpload({ name: appName, category, description, icon, installUrl });
    resetForm();
    onClose();
  };
      ...
      <div className="bg-slate-800 rounded-lg shadow-2xl p-8 w-full max-w-lg mx-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
           ...
           <div>
            <label htmlFor="installUrl" className="block text-sm font-medium text-slate-300">Link cài đặt</label>
            <input type="url" id="installUrl" value={installUrl} onChange={(e) => setInstallUrl(e.target.value)} placeholder="https://..." className="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-cyan-500 focus:border-cyan-500" required />
          </div>
          <div>
